        ...
        <div>
            ...
            <p class="text-gray-500 text-sm">
                @if(auth()->user()->role === 'admin')
                    {{-- Updated text since Admin can now Create reports too --}}
            ...
            <div class="flex min-h-full items-center justify-center p-4">
                <div class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:w-full sm:max-w-2xl">
                    ...
                    <div class="px-6 py-6 space-y-4">
                        ...
                        <div class="flex justify-between items-center pb-4 border-b border-gray-100">
                            <div>
                                ...
                                <p id="viewId" class="text-lg font-bold text-gray-900"></p>
                            </div>
                    ...
                    <div class="bg-gray-50 px-6 py-3 flex flex-row-reverse">
                        <button onclick="closeViewModal()" class="w-full inline-flex justify-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:w-auto">Close</button>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <form id="incidentForm" action="{{ route('incidents.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="px-6 py-6 max-h-[70vh] overflow-y-auto">
                            ...
                            <div class="mb-5">
                                ...
                                <input type="file" name="evidence[]" multiple id="inputEvidence" accept="image/*" class="block w-full text-sm text-gray-500
                                  file:mr-4 file:py-2 file:px-4
                                  file:rounded-full file:border-0
                                  file:text-sm file:font-semibold
                                ...
                                "/>
                                <p class="text-xs text-gray-400 mt-1">Select multiple files (PNG, JPG). Max 5MB each. (Adding files here will ADD to existing ones).</p>
                            </div>
...
<script>
    ...
    function openTimelineModal(id, stage, status, remarks, historyData) {
        ...
        // 2. SAFE SETUP: Admin Actions (Approve/Return)
        // We check if these elements exist before trying to change them.
        const approveForm = document.getElementById('approveForm');
        if (approveForm) {
            approveForm.action = "/incidents/" + id + "/status";
        }

        const returnForm = document.getElementById('returnForm');
        if (returnForm) {
            returnForm.action = "/incidents/" + id + "/status";
        }

        // 3. Render Steps (SIR -> PIR -> FIR)
        ...
        // 5. Update Button Text (Only runs if button exists)
        const nextBtn = document.getElementById('nextStageBtn');
        if(nextBtn) {
            if(stage === 'SIR') {
                nextBtn.innerHTML = 'Approve SIR & Move to PIR <i class="fa-solid fa-arrow-right ml-2"></i>';
            } else if(stage === 'PIR') {
                nextBtn.innerHTML = 'Approve PIR & Move to FIR <i class="fa-solid fa-arrow-right ml-2"></i>';
            } else {
                nextBtn.innerHTML = 'Finalize & Close Case <i class="fa-solid fa-lock ml-2"></i>';
            }
        }
        ...
        // 7. Show "Returned" Alert (Visible to everyone)
        if (status === 'Returned') {
            ...
            // Turn the current step Red to indicate an error
   ...
   function showHistoryContent(stageName) {
        ...
        if (record) {
            ...
            if (record.images && Array.isArray(record.images) && record.images.length > 0) {
                imagesHtml = `<div class="mt-4 pt-4 border-t border-slate-200">
                                <p class="text-[10px] text-slate-400 uppercase font-bold mb-2">Attached Evidence (${record.images.length})</p>
            ...
            displayArea.innerHTML = `
                <div class="bg-slate-50 border border-slate-200 rounded-xl overflow-hidden transition-all duration-300">
                    <div class="bg-slate-100 px-4 py-2 border-b border-slate-200 flex justify-between items-center">
                        <h4 class="font-bold text-slate-700 text-xs uppercase tracking-wider">
                            <i class="fa-solid fa-camera-retro mr-1"></i> ${stageName} Snapshot
                        </h4>
                        <span class="text-[10px] text-slate-400 italic">Archived on ${savedDate}</span>
                    </div>
                    ...
                    <div class="p-4 grid grid-cols-2 gap-4 text-sm">
                        ...
                        <div class="col-span-2 mt-2 pt-2 border-t border-slate-200">
                            <p class="text-[10px] text-slate-400 uppercase font-bold mb-1">Description</p>
                            <p class="text-slate-600 leading-relaxed whitespace-pre-wrap">${record.description}</p>
                        </div>
            ...
            displayArea.innerHTML = `
                <div class="p-6 text-center border-2 border-dashed border-slate-100 rounded-xl">
                    <p class="text-sm text-slate-400 italic">No recorded data for the ${stageName} stage yet.</p>
                </div>`;
        }
    }
